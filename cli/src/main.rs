...
use cloudstate_runtime::{
  blob_storage::{
    fs_store::FsBlobStore, in_memory_store::InMemoryBlobStore,
    CloudstateBlobStorage, CloudstateBlobStorageEngine,
  },
...
enum Cli {
  ...
  #[command(
    name = "gc",
    about = "Runs the garbage collector on a database file"
  )]
  Gc(GcArguments),
...
async fn main() {
  ...
  match Cli::parse() {
    ...
    Cli::Serve(CliArguments {
      ...
      let blob_storage_engine: Arc<dyn CloudstateBlobStorageEngine> =
        if memory_only {
          Arc::new(InMemoryBlobStore::new())
        } else {
      ...
      let blob_storage =
        CloudstateBlobStorage::new(blob_storage_engine.clone());
          ...
          move |evt: Result<notify::Event, notify::Error>| {
            ...
            if should_reload {
              ...
              Runtime::new().unwrap().block_on(async {
                if let Ok(new_classes) =
                  fs::read_to_string(&pre_cloned_filename)
                {
...
async fn handler(
  ...
  let mut service: axum::routing::RouterIntoService<Body> =
    router.into_service();
