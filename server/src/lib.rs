...
use cloudstate_runtime::{
  extensions::cloudstate::ReDBCloudstate, v8_string_key,
};
use deno_core::JsRuntime;
use deno_core::*;
use deno_core::{
  futures::future::poll_fn, resolve_import, ModuleLoadResponse, ModuleLoader,
  ...
  let headers = headers
    ...
    .map(|(key, value)| {
      format!(
        ...
        serde_json::to_string(&value.to_str().unwrap_or_default().to_string())
          .unwrap()
      )
    })
    .collect::<Vec<String>>()
