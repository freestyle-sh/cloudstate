const baseArray = [{ a: 1 }, { a: "3" }, { a: 5 }, { b: 6 }, { c: "7" }];
...

{
  ...
  for (let i = 0; i < root.value.length; i++) {
    if (root.value.at(i) !== baseArray[i]) {
      throw new Error(
        `value mismatch at index ${i}: ${root.value.at(i)} !== ${baseArray[i]}`
      );
    }
  }
...

{
  ...
  if (root.value.length !== baseArray.length + 1) {
    throw new Error(`root.value should have length ${baseArray.length + 1}`);
  }
  if (root.value.at(baseArray.length) !== newObject) {
    throw new Error(
      `value mismatch at index ${baseArray.length}: ${root.value.at(
        baseArray.length
      )} !== ${newObject}`
    );
  }
...

{
  ...
  if (root.value.at(1) !== baseArray[2]) {
    throw new Error(
      `value mismatch at index 1: ${root.value.at(1)} !== ${baseArray[2]}`
    );
  }
...

{
  ...
  if (root.value.at(0) !== baseArray[3]) {
    throw new Error(
      `value mismatch at index 0: ${root.value.at(0)} !== ${baseArray[3]}`
    );
  }
  if (root.value.at(1) !== baseArray[4]) {
    throw new Error(
      `value mismatch at index 1: ${root.value.at(1)} !== ${baseArray[4]}`
    );
  }
